...
@app.route('/api/v0.1/arena/add', methods=['GET', 'POST'])
def add_arenas():
...
@app.route('/api/v0.1/arena/delete/<int:arena_id>', methods=['DELETE'])
def delete_arena(arena_id):
  arena = session.query(Arena).delete(arena_id)
  return jsonify({"deleted":"success"})
  
@app.route('/api/v0.1/county', methods=['GET'])
def get_counties():
...
@app.route('/api/v0.1/county/query/size/<float:size>', methods=['GET'])
...
@app.route('/api/v0.1/county/<int:county_id>', methods=['GET'])
...
@app.route('/api/v0.1/county/<county_name>', methods=['GET'])
...
def get_states():
	...
	if "geometry" in request.args.keys():
		if request.args["geometry"] == '1' or request.args["geometry"] == 'True':
                    data = [{"type": "Feature", "properties": {"state": state.name, "id": state.id}, "geometry": {"type": "MultiPolygon","coordinates": [shapely.geometry.geo.mapping(to_shape(state.geom))["coordinates"]]}, } for state in states]

	return jsonify({"type": "FeatureCollection", "features": data})


@app.route('/api/v0.1/state/<int:state_id>', methods=['GET'])
...
@app.route('/api/v0.1/state/<int:state_id>/within', methods=['GET'])
...
@app.route('/api/v0.1/state/<state_name>', methods=['GET'])
...
@app.route('/api/v0.1/district', methods=['GET'])
def get_districts():
...
@app.route('/api/v0.1/district/<int:district_id>', methods=['GET'])
...
@app.route('/api/v0.1/district/<district_name>', methods=['GET'])
